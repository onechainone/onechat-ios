...
- (LZBroadcastView*)mainView
{
    if (!_mainView) {
        ...
        [_mainView addGestureRecognizer:longPressGest];
    }
    return _mainView;
}
...
-(void)longPressView:(UILongPressGestureRecognizer *)longPressGest{
    ...
    //discern_qrcode button_cancel
    UIAlertAction *showAllInfoAction = [UIAlertAction actionWithTitle:NSLocalizedString(@"heading_qrcode_to_img", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        lhScanQCodeViewController * sqVC = [[lhScanQCodeViewController alloc]init];
        sqVC.succ = ^(NSString *str) {
            NSDictionary *para = @{
                                   @"str":str
                                   };
            [[NSNotificationCenter defaultCenter] postNotificationName:@"QRCODE_RECEIVE" object:nil userInfo:para];
        };
        ...
        LZPicture *pic = self.pictures[_currentIndex];
        UIImage *image = pic.image;
        // 2.直接开始识别图片,获取图片特征
        ...
        if (features.count >=1) {
            ...
            [sqVC accordingQcode:scannedResult];
            [self disappear];
        }
        else{
            //"there_is_no_qrcode" = "该图片没有包含一个二维码！";
            //button_ok
            //message_title
            UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:NSLocalizedString(@"message_title", nil) message:NSLocalizedString(@"there_is_no_qrcode", nil) delegate:nil cancelButtonTitle:NSLocalizedString(@"button_ok", nil) otherButtonTitles:nil, nil];
            [alertView show];
            
        }
    }];
    
    UIAlertAction *pickAction = [UIAlertAction actionWithTitle:NSLocalizedString(@"image_save_to_album", @"保存图片") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        
        LZPicture *picture = (LZPicture*)self.pictures[self.currentIndex];
        
        if (picture.image) {
            
//            UIImageWriteToSavedPhotosAlbum(picture.image, self, @selector(image:didFinishSavingWithError:contextInfo:), (__bridge_retained void *)(sender));
            UIImageWriteToSavedPhotosAlbum(picture.image, self, @selector(image:didFinishSavingWithError:contextInfo:), (__bridge void *)self);
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo
{
    ...
    NSString *msg = nil ;
    if(error != NULL){
        msg = NSLocalizedString(@"image_save_fail", @"");
    }else{
        msg = NSLocalizedString(@"image_save_success", @"");
    }

    [self showHudInView:self.view hint:NSLocalizedString(@"image_save_to_album", @"保存至相册")];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        
        [self hideHud];
        if ([button isKindOfClass:[UIButton class]]) {
            [self showHint:msg inView:self.view];
