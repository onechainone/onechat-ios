...
- (instancetype)initWithConversation:(ONEConversation *)conversation
{
    ...
    if (self) {
        ...
        _title = _conversation.conversationTitle;
        if (conversation.type == ONEConversationTypeChat) {
            
            _avatarImage = [UIImage imageNamed:@"EaseUIResource.bundle/user"];
        }
        else{
            
            _avatarImage = [UIImage imageNamed:@"group_default_avatar"];
        }
    }
    _unreadCount = _conversation.unreadMessagesCount;
