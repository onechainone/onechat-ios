...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (model) {
        if (_delegate && [_delegate respondsToSelector:@selector(userListViewController:didSelectUserModel:)]) {
            ...
            EaseMessageViewController *viewController = [[EaseMessageViewController alloc] initWithConversationChatter:model.buddy conversationType:ONEConversationTypeChat];
    ...
    [[ONEChatClient sharedClient] getFriendListWithCompletion:^(ONEError *error, NSArray *list) {
        ...
        if (!error && list) {
            ...
            for (NSInteger i = 0; i < list.count; i++) {
                ...
                ONEFriendModel *friendModel = list[i];
                NSString *buddy = friendModel.account_name;
                id<IUserModel>model = nil;
