...
@interface EaseChatToolbar()<UITextViewDelegate, EMFaceDelegate, AGEmojiKeyboardViewDataSource, AGEmojiKeyboardViewDelegate>
...
@property (nonatomic, strong) NSString *recentlyEmoji;
...
- (void)_setupSubviews
{
    ...
    _backgroundImageView.themeMap = @{
                                      BGColorName:@"chat_toolbar_bg_color"
                                      };
...
//    _inputTextView.layer.borderColor = [UIColor colorWithWhite:0.8f alpha:1.0f].CGColor;
    _inputTextView.layer.borderColor = THMColor(@"input_text_border_color").CGColor;
    _inputTextView.layer.borderWidth = 0.55f;
...
//    communityButton.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
//    [communityButton setImage:[UIImage imageNamed:@"show_community"] forState:UIControlStateNormal];
//    [communityButton addTarget:self action:@selector(communityButtonAction:) forControlEvents:UIControlEventTouchUpInside];
//    EaseChatToolbarItem *commItem = [[EaseChatToolbarItem alloc] initWithButton:communityButton withView:nil];
    //语音
    //change input type
    UIButton *styleChangeButton = [[UIButton alloc] init];
    styleChangeButton.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
    [styleChangeButton setImage:[UIImage imageNamed:@"One_Record_Item"] forState:UIControlStateNormal];
    [styleChangeButton setImage:[UIImage imageNamed:@"One_Record_Keyboard"] forState:UIControlStateSelected];
...
//    if (_chatBarType == EMChatToolbarTypeChat) {
        [self setInputViewLeftItems:@[styleItem]];
//    } else {
//        [self setInputViewLeftItems:@[commItem,styleItem]];
//        _lineView = [[UIView alloc] init];
//        _lineView.backgroundColor = RGBACOLOR(234, 234, 234, 1);
//        _lineView.frame = CGRectMake(CGRectGetMaxX(communityButton.frame) + 7, 0, 1, self.height);
...
//    }
    ...
    [self.recordButton setTitle:NSLocalizedString(@"button_pushtotalk", @"") forState:UIControlStateNormal];
    [self.recordButton setTitle:NSLocalizedString(@"release_to_close", @"") forState:UIControlStateHighlighted];
    ...
    [self.faceButton setImage:[UIImage imageNamed:@"emoji_chat"] forState:UIControlStateNormal];
    [self.faceButton setImage:[UIImage imageNamed:@"emoji_chat"] forState:UIControlStateHighlighted];
//    [self.faceButton setImage:[UIImage imageNamed:@"ToolViewKeyboard"] forState:UIControlStateSelected];
//    ToolViewKeyboardOne_Record_Keyboard EaseUIResource.bundle/chatBar_keyboard
    [self.faceButton setImage:[UIImage imageNamed:@"One_Record_Keyboard"] forState:UIControlStateSelected];
    ...
    [self.moreButton setImage:THMImage(@"chat_tool_more") forState:UIControlStateNormal];
    [self.moreButton setImage:THMImage(@"chat_tool_more") forState:UIControlStateHighlighted];
//    [self.moreButton setImage:[UIImage imageNamed:@"ToolViewKeyboard"] forState:UIControlStateSelected];

    [self.moreButton setImage:[UIImage imageNamed:@"One_Record_Keyboard"] forState:UIControlStateSelected];
...
{
    if (_recordView == nil) {
        _recordView = [[EaseRecordView alloc] initWithFrame:CGRectMake(90, 130, 200, 200)];
    }
...
//    return _faceView;
//}
- (AGEmojiKeyboardView *)faceView
{
    if (_faceView == nil) {
        _faceView = [[AGEmojiKeyboardView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_toolbarView.frame), self.frame.size.width, 216) dataSource:self];
        _faceView.delegate = self;
        _faceView.backgroundColor = [UIColor colorWithRed:240 / 255.0 green:242 / 255.0 blue:247 / 255.0 alpha:1.0];
        _faceView.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
        [_faceView setValue:[UIColor lightGrayColor] forKeyPath:@"segmentsBar.tintColor"];
    }
...
- (void)_willShowBottomHeight:(CGFloat)bottomHeight
{
    ...
    if (bottomHeight == 0) {
        if (_delegate && [_delegate respondsToSelector:@selector(didChatToolBarClose)]) {
            [_delegate didChatToolBarClose];
        }
...
- (void)inputViewTextExceed:(BOOL)isExceed
{
    if (isExceed) {
        _inputTextView.layer.borderColor = [UIColor redColor].CGColor;
    } else {
        _inputTextView.layer.borderColor = [UIColor colorWithWhite:0.8f alpha:1.0f].CGColor;
    }
}
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    ...
    if (_chatBarType != EMChatToolBarTypeComment && [str length] > KMAX_INPUT_COUNT) {
        
        NSRange rangeIndex = [str rangeOfComposedCharacterSequenceAtIndex:KMAX_INPUT_COUNT];
        if (rangeIndex.length == 1) {
            textView.text = [str substringToIndex:KMAX_INPUT_COUNT];
            [self inputViewTextExceed:YES];
        } else {
            
            NSRange rangeRange = [str rangeOfComposedCharacterSequencesForRange:NSMakeRange(0, KMAX_INPUT_COUNT)];
            textView.text = [str substringWithRange:rangeRange];
            [self inputViewTextExceed:NO];
...
{
    if (_chatBarType != EMChatToolBarTypeComment && textView.text.length > KMAX_INPUT_COUNT) {
        textView.text = [textView.text substringToIndex:KMAX_INPUT_COUNT];
        [self inputViewTextExceed:YES];
    } else {
...
//    //暂时不能交易can_not_trade
//    _HUD.labelText = NSLocalizedString(@"feature_come_soon", nil);
...
- (void)communityButtonAction:(id)sender
{
    if (_delegate && [_delegate respondsToSelector:@selector(didShowCommunityStyle:)]) {
        [_delegate didShowCommunityStyle:self];
    }
}
...
#pragma mark - 自定义Emoji键盘


- (void)emojiKeyBoardView:(AGEmojiKeyboardView *)emojiKeyBoardView didUseEmoji:(NSString *)emoji {
    
    [self selectedFacialView:emoji isDelete:NO];
}
...
- (UIColor *)randomColor {
    ...
    return [UIColor colorWithHex:THEME_COLOR];
}
...
//    return img;
    NSString *imageName = [self imageNameFromAGECategory:category isSelected:YES];
...
//    [img imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
//    return img;
//    return [self imageNameFromAGECategory:category isSelected:NO];
    NSString *imageName = [self imageNameFromAGECategory:category isSelected:NO];
    UIImage *image = [UIImage imageNamed:imageName];
    [image imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
...
//    EaseUIResource.bundle/faceDelete
    UIImage *img = [UIImage imageNamed:@"sym_keyboard_delete_holo_dark"];
    [img imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
    return img;
}

- (NSString *)imageNameFromAGECategory:(AGEmojiKeyboardViewCategoryImage)category isSelected:(BOOL)isSelected
{
    NSString *imageName = @"";
    if (!isSelected) {
        ...
        switch (category) {
            case AGEmojiKeyboardViewCategoryImageRecent:
                imageName = @"ic_emoji_recent_light_normal";
                break;
                case AGEmojiKeyboardViewCategoryImageFace:
                imageName = @"ic_emoji_people_light_normal";
                break;
                case AGEmojiKeyboardViewCategoryImageBell:
                imageName = @"ic_emoji_objects_light_normal";
                break;
                case AGEmojiKeyboardViewCategoryImageFlower:
                imageName = @"ic_emoji_nature_light_normal";
                break;
                case AGEmojiKeyboardViewCategoryImageCar:
                imageName = @"ic_emoji_places_light_normal";
                break;
                case AGEmojiKeyboardViewCategoryImageCharacters:
                imageName = @"ic_emoji_symbols_light_activated";
                break;
            default:
                break;
        }
        ...
        switch (category) {
            case AGEmojiKeyboardViewCategoryImageRecent:
                imageName = @"ic_emoji_recent_light_activated";
                break;
            case AGEmojiKeyboardViewCategoryImageFace:
                imageName = @"ic_emoji_people_light_activated";
                break;
            case AGEmojiKeyboardViewCategoryImageBell:
                imageName = @"ic_emoji_objects_light_activated";
                break;
            case AGEmojiKeyboardViewCategoryImageFlower:
                imageName = @"ic_emoji_nature_light_activated";
                break;
            case AGEmojiKeyboardViewCategoryImageCar:
                imageName = @"ic_emoji_places_light_activated";
                break;
            case AGEmojiKeyboardViewCategoryImageCharacters:
                imageName = @"ic_emoji_symbols_light_activated";
                break;
