...
@property (nonatomic, strong) UIButton *atAllButton;
...
- (void)setupSubviewsForType:(EMChatToolbarType)type
{
    ...
    [_photoButton setTitle:NSLocalizedString(@"attach_picture", @"") forState:UIControlStateNormal];
    
    [_photoButton addTarget:self action:@selector(photoAction) forControlEvents:UIControlEventTouchUpInside];
    _photoButton.tag = MOREVIEW_BUTTON_TAG;
    [self resetButtonTitle:_photoButton];
    [_scrollview addSubview:_photoButton];
    
    _takePicButton =[UIButton buttonWithType:UIButtonTypeCustom];
//
    [_takePicButton setFrame:CGRectMake(insets * 2 + CHAT_BUTTON_SIZE, 10, CHAT_BUTTON_SIZE , CHAT_BUTTON_HEIGHT)];
    ...
    [_takePicButton setTitle:NSLocalizedString(@"attach_take_pic", @"") forState:UIControlStateNormal];
    [self resetButtonTitle:_takePicButton];
    ...
    [_locationButton setFrame:CGRectMake(insets * 3 + CHAT_BUTTON_SIZE * 2, 10, CHAT_BUTTON_SIZE , CHAT_BUTTON_HEIGHT)];
    ...
    [_locationButton setTitle:NSLocalizedString(@"attach_location", @"") forState:UIControlStateNormal];
    [self resetButtonTitle:_locationButton];
    ...
    if (type == EMChatToolbarTypeChat) {
        ...
        [_audioCallButton setImage:[UIImage imageNamed:@"icon_Transfer"] forState:UIControlStateNormal];
        [_audioCallButton setImage:[UIImage imageNamed:@"icon_Transfer"] forState:UIControlStateHighlighted];
//        [_audioCallButton setTitle:NSLocalizedString(@"fast_transfer", @"") forState:UIControlStateNormal];
        [self resetButtonTitle:_audioCallButton];
        [_audioCallButton addTarget:self action:@selector(transferAction) forControlEvents:UIControlEventTouchUpInside];
...
- (void)insertItemWithImage:(UIImage *)image highlightedImage:(UIImage *)highLightedImage title:(NSString *)title
{
    ...
    [moreButton setFrame:CGRectMake(page * CGRectGetWidth(self.frame) + insets * (col + 1) + CHAT_BUTTON_SIZE * col, INSETS + INSETS * 2 * row + CHAT_BUTTON_HEIGHT * row, CHAT_BUTTON_SIZE , CHAT_BUTTON_HEIGHT)];
    ...
    moreButton.titleLabel.font = [UIFont fontWithName:FONT_NAME_REGULAR size:12];
    [moreButton setTitleColor:DEFAULT_BLACK_COLOR forState:UIControlStateNormal];
    [self resetButtonTitle:moreButton];
...
- (void)resetButtonTitle:(UIButton *)button
{
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, button.width + 20, 15)];
    label.font = [UIFont fontWithName:FONT_NAME_REGULAR size:12.f];
//    label.textColor = DEFAULT_BLACK_COLOR;
    label.themeMap = @{
    ...
    [label setCenterX:button.centerX];
    [label setY:(CGRectGetMaxY(button.frame) + 4)];
}
