...
NSString *const EaseMessageCellIdentifierRecvTransfer = @"EaseMessageCellRecvTransfer";
NSString *const EaseMessageCellIdentifierRecvRedpacket = @"EaseMessageCellRecvRedpacket";
...
NSString *const EaseMessageCellIdentifierSendTransfer = @"EaseMessageCellSendTransfer";
NSString *const EaseMessageCellIdentifierSendRedpacket = @"EaseMessageCellSendRedpacket";

static const CGFloat kRedpacketMaxWidth = 227.f;

@interface EaseMessageCell()
{
    ONEMessageBodyType _messageType;
}
...
+ (void)initialize
{
    ...
    cell.bubbleMaxWidth = KScreenW - 2 * 60;
    ...
    cell.messageTextFont = /**[UIFont systemFontOfSize:15]*/ [UIFont fontWithName:FONT_NAME_LIGHT size:15];
...
- (void)_setupSubviewsWithType:(ONEMessageBodyType)messageType
    ...
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] && [self isCustomBubbleView:model]) {
        ...
        switch (messageType) {
            ...
            case ONEMessageBodyTypeImage:
            {
                ...
                _bubbleView.imageView.image = [UIImage imageNamed:@"img_download_fail"];
            }
                break;
            case ONEMessageBodyTypeLocation:
            {
                ...
                UIImage *image = [[UIImage imageNamed:@"one_msg_loc"] stretchableImageWithLeftCapWidth:10 topCapHeight:10];
                _bubbleView.locationImageView.image = image;
            ...
            {
                _bubbleView.duration = model.mediaDuration;
                [_bubbleView setupVoiceBubbleView];
                if (model.isSender) {
                    _bubbleView.voiceDurationLabel.textColor = [UIColor whiteColor];
                    _bubbleView.voiceDurationLabel.font = [UIFont fontWithName:FONT_NAME_REGULAR size:16.f];
                } else {
                    _bubbleView.voiceDurationLabel.textColor = RGBACOLOR(131, 131, 131, 1);
                    _bubbleView.voiceDurationLabel.font = [UIFont fontWithName:FONT_NAME_REGULAR size:16.f];
                }
            ...
            {
                [_bubbleView setupTransferBubbleView];
            }
                break;
                case ONEMessageBodyTypeRedpacket:
            {
                [_bubbleView setupRedpacketBubbleView];
                self.bubbleMaxWidth = kRedpacketMaxWidth;
            }
                break;
            default:
                break;
...
//    UILongPressGestureRecognizer *longPress = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(avatarViewLongPressed:)];
        ...
        switch (model.bodyType) {
            case ONEMessageBodyTypeText:
            {
                if (model.isSender) {
                    ...
                    _bubbleView.textLabel.attributedText = [[EaseEmotionEscape sharedInstance] attStringFromTextForChatting:model.text textFont:self.messageTextFont textColor:THMColor(@"chat_sender_text_color")];
                } else {
                    _bubbleView.textLabel.attributedText = [[EaseEmotionEscape sharedInstance] attStringFromTextForChatting:model.text textFont:self.messageTextFont textColor:THMColor(@"chat_recv_text_color")];
                }
...
//                        [_bubbleView.imageView sd_setImageWithURL:[NSURL URLWithString:_model.fileURLPath] placeholderImage:[UIImage imageNamed:_model.failImageName]];
                        if (_model.exceedImage) {
                            _bubbleView.imageView.image = _model.exceedImage;
                        }
                    } else {
            ...
            case ONEMessageBodyTypeVoice:
            {
                if (_bubbleView.voiceImageView) {
                    if ([self.sendMessageVoiceAnimationImages count] > 0 && [self.recvMessageVoiceAnimationImages count] > 0) {
                        ...
                        self.bubbleView.voiceImageView.image = self.model.isSender ?[UIImage imageNamed:@"Voice_Self"]: [UIImage imageNamed:@"Voice_Other"];
                    }
                }
            ...
            case ONEMessageBodyTypeTransfer:
            {
                _bubbleView.amountLabel.text = _model.amount;
                _bubbleView.symbolLabel.text = _model.symbol;
            }
                break;
            case ONEMessageBodyTypeRedpacket:
            {
                if ([RedPacketMngr redpacketStatus:model.red_id]) {
                    ...
                    _bubbleView.redBgView.image = [UIImage imageNamed:@"Redpacket_bubble_got"];
                } else {
                    
                    _bubbleView.redBgView.image = [UIImage imageNamed:@"Redpacket_bubble"];
                }
                _bubbleView.msgLabel.text = _model.red_msg;
            }
                break;
            default:
                break;
...
- (void)setSendBubbleBackgroundImageDic:(NSDictionary *)sendBubbleBackgroundImageDic
{
    _sendBubbleBackgroundImageDic = sendBubbleBackgroundImageDic;
}

- (void)setRecvBubbleBackgroundImageDic:(NSDictionary *)recvBubbleBackgroundImageDic
{
    _recvBubbleBackgroundImageDic = recvBubbleBackgroundImageDic;
}
...
//- (void)avatarViewLongPressed:(UITapGestureRecognizer *)longPress
//{
//    if (longPress.state == UIGestureRecognizerStateBegan) {
//
//        if ([_delegate respondsToSelector:@selector(avatarViewPressed:)]) {
//
//            [_delegate avatarViewPressed:_model];
//        }
//    }
//}
    ...
    if (model.isSender) {
        switch (model.bodyType) {
            ...
            case ONEMessageBodyTypeFile:
                ...
                break;
            case ONEMessageBodyTypeTransfer:
                cellIdentifier = EaseMessageCellIdentifierSendTransfer;
                break;
            case ONEMessageBodyTypeRedpacket:
                cellIdentifier = EaseMessageCellIdentifierSendRedpacket;
                break;
            default:
                break;
        }
        ...
        switch (model.bodyType) {
            ...
            case ONEMessageBodyTypeFile:
                ...
                break;
            case ONEMessageBodyTypeTransfer:
                cellIdentifier = EaseMessageCellIdentifierRecvTransfer;
                break;
            case ONEMessageBodyTypeRedpacket:
                cellIdentifier = EaseMessageCellIdentifierRecvRedpacket;
                break;
            default:
                break;
        }
...
+ (CGFloat)cellHeightWithModel:(id<IMessageModel>)model
{
    ...
    switch (model.bodyType) {
        case ONEMessageBodyTypeText:
        {
            NSAttributedString *text = [[EaseEmotionEscape sharedInstance] attStringFromTextForChatting:model.text textFont:cell.messageTextFont textColor:nil];
            ...
            height += (rect.size.height > 23 ? rect.size.height : 23) + 10;
        ...
        case ONEMessageBodyTypeVideo:
        {
            ...
            if (retSize.width == 0 && retSize.height == 0) {
                ...
                retSize = model.exceedImage.size;
            }
            if (retSize.width == 0 && retSize.height == 0) {
            ...
            case ONEMessageBodyTypeTransfer:
        {
            ...
            case ONEMessageBodyTypeRedpacket:
        {
            UIImage *image = [UIImage imageNamed:@"Redpacket_bubble"];
            CGSize size = image.size;
            height = size.height;
        }
        default:
...
- (void)prepareForReuse
{
    ...
    if (_model.message.body.type == ONEMessageBodyTypeImage) {
