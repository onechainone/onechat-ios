...
{
    [self showBrowserWithImages:imageArray atIndex:0];
}
...
- (void)showBrowserWithImages:(NSArray *)imageArray atIndex:(NSUInteger)index
{
    ...
    for (MWPhoto *p in self.photos) {
        if (p.image != nil) {
            ...
            [pArray addObject:p.photoURL];
        }
    }
    LZPictureBrowser *mvc = [[LZPictureBrowser alloc] init];
    [mvc showWithPictureURLs:pArray atIndex:index];
}
                      ...
                      updateViewCompletion:(void (^)(EaseMessageModel *prevAudioModel, EaseMessageModel *currentAudioModel))updateCompletion
{
    ...
    if(messageModel.bodyType == ONEMessageBodyTypeVoice)
    {
        ...
        if (isPlaying) {
            ...
            if (!messageModel.isMediaPlayed) {
                ...
                ONEMessage *chatMessage = messageModel.message;
                
                [ONEChatClient markVoiceMessageAsPlayed:chatMessage.messageId];
            }
        }
...
- (EaseMessageModel *)stopMessageAudioModel
{
    ...
    if (self.audioMessageModel.bodyType == ONEMessageBodyTypeVoice) {
