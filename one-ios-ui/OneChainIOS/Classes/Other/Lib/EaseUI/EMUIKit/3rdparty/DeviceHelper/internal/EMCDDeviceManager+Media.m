...
- (void)asyncStartRecordingWithFileName:(NSString *)fileName
    ...
    if ([self isRecording]) {
        if (completion) {
            error = [NSError errorWithDomain:NSLocalizedString(@"error_record_not_stop", @"")
    ...
    if (!fileName || [fileName length] == 0) {
        error = [NSError errorWithDomain:NSLocalizedString(@"error_path_not_found", @"")
                                    code:-1
                                userInfo:nil];
        completion(error);
        return ;
...
-(void)asyncStopRecordingWithCompletion:(void(^)(NSString *recordPath,
    ...
    if(![self isRecording]){
        if (completion) {
            error = [NSError errorWithDomain:NSLocalizedString(@"error_record_not_begin", @"")
    ...
    if([_recorderEndDate timeIntervalSinceDate:_recorderStartDate] < [EMCDDeviceManager recordMinDuration]){
        if (completion) {
            error = [NSError errorWithDomain:NSLocalizedString(@"error_record_too_short", @"")
        ...
        if (completion) {
            if (recordPath) {
                ...
                if (convertResult) {
                    ...
                    error = [NSError errorWithDomain:NSLocalizedString(@"error_format_fail", @"")
                                                code:EMErrorFileTypeConvertionFailure
                                            userInfo:nil];
                }
