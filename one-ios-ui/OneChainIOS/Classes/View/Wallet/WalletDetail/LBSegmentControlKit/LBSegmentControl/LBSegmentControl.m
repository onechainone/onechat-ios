...
- (void)layoutSubviews {
    ...
    if (self.selectedIndex > 0) {
        self.currentItem = [self.segementScrollView viewWithTag:1995 + self.selectedIndex];
    } else {
    ...
    // 创建默认的控制器试图
    [self addCtrlViewToScrollViewWithCtrlIndex:self.currentItem.tag - 1995];
    ...
    // 设置底部条的宽度
    if (_isBottomViewScale) {
        self.bottomView.width = _bottomScaledWidth > 0 ? _bottomScaledWidth : self.currentItem.width;
...
- (instancetype)initStaticTitlesWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _isBottomViewScale = YES;
        _bottomScaledWidth = 40;
        _selectedIndex = 0;
        [self initDefault];
...
- (void)creatSegementScrollView {
    ...
    self.segementScrollView.keyboardDismissMode = UIScrollViewKeyboardDismissModeOnDrag;
...
//    self.scrollCtrlView.backgroundColor = [UIColor whiteColor];
    ...
    self.scrollCtrlView.keyboardDismissMode = UIScrollViewKeyboardDismissModeOnDrag;
...
//        segment.tradeThemeMap = self.tradeThemeMap;
        segment.is_trade = self.is_trade;
        segment.titleNormalColor = self.titleNormalColor;
        segment.titleSelectColor = self.titleSelectColor;
...
- (void)setSelectedIndex:(NSUInteger)selectedIndex
{
    ...
    for (UIView *subView in self.segementScrollView.subviews) {
        ...
        if ([subView isKindOfClass:[LBSegment class]] && (((LBSegment *)subView).tag - 1995 == selectedIndex)) {
            [self itemAction:(LBSegment *)subView];
        }
    }
}
...
- (void)itemAction:(LBSegment *)segment {
    ...
    //NOTIFICATION_SEGMENT_CHANGE
    [[NSNotificationCenter defaultCenter] postNotificationName:NOTIFICATION_SEGMENT_CHANGE object:nil];
...
- (void)changeSelectProgressWithCurrentIndex:(NSInteger)currentIndex {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        if (_isBottomViewScale) {
            self.bottomView.width = _bottomScaledWidth > 0 ? _bottomScaledWidth : self.currentItem.width;
            differenceX = self.nextItem.centerX - self.currentItem.centerX;
            self.bottomView.centerX = self.currentItem.centerX + (differenceX * (1 - self.currentSelectProgress));
        } else {
