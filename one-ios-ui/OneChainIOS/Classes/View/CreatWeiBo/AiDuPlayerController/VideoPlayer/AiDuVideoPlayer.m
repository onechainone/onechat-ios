...
@property (nonatomic, strong) AiDuSlider *slider;
...
@implementation AiDuVideoPlayer
...
- (void)endPlay:(NSNotification*)info{
...
#pragma mark 显示底部控件和播放按钮
- (void)show {
    self.barHiden = YES;
    if (self.showBottomBlock) {
        self.showBottomBlock(self);
    }
...
#pragma mark 拖拽进度条
- (void)sliderValueChange:(AiDuSlider *)slider {
...
//            [MTool showRequestFailedAlert:@"视频审核中" inView:self];
            double delayInSeconds = 1.0;
            dispatch_time_t popTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayInSeconds * NSEC_PER_SEC));
            dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
                
                [[NSNotificationCenter defaultCenter] postNotificationName:@"AVPlayerStatusFailed" object:nil];
            });
...
- (UIView *)bottomBar {
    if (!_bottomBar) {
        ...
        playPauseButton.layer.opacity = 0.0f;
        playPauseButton.contentMode = UIViewContentModeCenter;
        playPauseButton.translatesAutoresizingMaskIntoConstraints = NO;
        [playPauseButton setBackgroundImage:[UIImage imageNamed:@"ykPlayButton"] forState:UIControlStateNormal];
        [playPauseButton setBackgroundImage:[UIImage imageNamed:@"ykStopButton"] forState:UIControlStateSelected];
        [playPauseButton addTarget:self action:@selector(playOrPause:) forControlEvents:UIControlEventTouchDown];
        
        [_bottomBar addSubview:playPauseButton];
        self.playOrPauseBtn = playPauseButton;
        
        NSLayoutConstraint *playBtnWidth = [NSLayoutConstraint constraintWithItem:playPauseButton attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeWidth multiplier:1.0f constant:40.0f];
        NSLayoutConstraint *playBtnHeight = [NSLayoutConstraint constraintWithItem:playPauseButton attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeHeight multiplier:1.0f constant:40.0f];
        NSLayoutConstraint *playBtnLeft = [NSLayoutConstraint constraintWithItem:playPauseButton attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:_bottomBar attribute:NSLayoutAttributeLeft multiplier:1.0f constant:0];
        NSLayoutConstraint *playBtnCenterY = [NSLayoutConstraint constraintWithItem:playPauseButton attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:_bottomBar attribute:NSLayoutAttributeCenterY multiplier:1.0f constant:0];
        [_bottomBar addConstraints:@[playBtnWidth, playBtnHeight, playBtnLeft, playBtnCenterY]];
        ...
        NSLayoutConstraint *label1Left = [NSLayoutConstraint constraintWithItem:label1 attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:playPauseButton attribute:NSLayoutAttributeRight multiplier:1.0f constant:2];
...
//        [fullScreenBtn setImage:[UIImage imageNamed:@"ImageResources.bundle/btn_zoom_in"] forState:UIControlStateSelected];
        ...
        [fullScreenBtn setImage:[UIImage imageNamed:@"playScreen"] forState:UIControlStateSelected];
        ...
        AiDuSlider *slider = [[AiDuSlider alloc] init];
        slider.value = 0.0f;
        ...
        self.slider = slider;
        __weak typeof(self) weakSelf = self;
        slider.valueChangeBlock = ^(AiDuSlider *slider){
            [weakSelf sliderValueChange:slider];
        };
        slider.finishChangeBlock = ^(AiDuSlider *slider){
            [weakSelf finishChange];
        };
        slider.draggingSliderBlock = ^(AiDuSlider *slider){
            [weakSelf dragSlider];
        };
...
//        [_playOrPauseBtn setBackgroundImage:[UIImage imageNamed:@"ImageResources.bundle/pause"] forState:UIControlStateSelected];
        ...
        [_playOrPauseBtn setBackgroundImage:[UIImage imageNamed:@"ykPlayButton"] forState:UIControlStateNormal];
        [_playOrPauseBtn setBackgroundImage:[UIImage imageNamed:@"ykStopButton"] forState:UIControlStateSelected];
        //
