...
- (UITableView *)tableView
{
    if (_tableView == nil) {
        ...
        _tableView.sectionIndexColor = THMColor(@"theme_special_color");
...
- (NSArray *)sortRecords:(NSArray *)recordArray
{
    ...
    if (_objectComparisonStringBlock) {
        for (id object in recordArray) {
            ...
            if ([object isKindOfClass:[ONEFriendModel class]])
            {
            
                ONEFriendModel *model = (ONEFriendModel *)object;
                if (model) {
                    NSString *firstLetter = [EaseChineseToPinyin pinyinFromChineseString:model.showName];
                    NSString *objStr = _objectComparisonStringBlock(firstLetter);
    ...
    if (_comparisonObjectSelector) {
        ...
        for (int i = 0; i < [sortedArray count]; i++) {
            NSArray *tmpArray = [[sortedArray objectAtIndex:i] sortedArrayUsingComparator:^NSComparisonResult(ONEFriendModel *obj1, ONEFriendModel *obj2) {
                
                
                NSString *nick1 = obj1.showName;
                NSString *firstLetter1 = [EaseChineseToPinyin pinyinFromChineseString:nick1];
                if (firstLetter1.length > 0) {
                    firstLetter1 = [[firstLetter1 substringToIndex:1] uppercaseString];
                }
                
                NSString *nick2 = obj2.showName;
                NSString *firstLetter2 = [EaseChineseToPinyin pinyinFromChineseString:nick2];
                if (firstLetter2.length > 0) {
                    firstLetter2 = [[firstLetter2 substringToIndex:1] uppercaseString];
                }
                return weakSelf.comparisonObjectSelector(firstLetter1, firstLetter2);
            }];
...
- (void)doneAction:(id)sender
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(viewController:didFinishSelectedSources:)]) {
        ...
        for (NSIndexPath *indexPath in self.selectedIndexPaths) {
            ONEFriendModel *model = [[self.dataSource objectAtIndex:indexPath.section] objectAtIndex:indexPath.row];
            if (model.account_name) {
                [resultArray addObject:model.account_name];
            }
        }
