...
#define kCURRENT_USERNAME [ONEChatClient homeAccountName]
...
- (UserProfileEntity*)getUserProfileByUsername:(NSString*)username
{
    ...
    NSArray* list = [username componentsSeparatedByString:@"."];
    
    if( list != nil && list.count == 3 ) {
    ...
    WSAccountInfo* info = nil;
    
    if( isUserName ) {
        
        info =     [ONEChatClient accountInfoWithName:username];

        
    } else {
        
        info =     [ONEChatClient accountInfoWithId :username];

    }
    
    
    
    UserProfileEntity* entity = [UserProfileEntity initWithAccountInfo:info];
...
+ (instancetype) initWithAccountInfo:(WSAccountInfo*) info {
    
    if( info == nil ) return nil;
    
    UserProfileEntity* entity = [[UserProfileEntity alloc] init];
    
    entity.username = info.name;
    entity.nickname = [info accountNickName];
    if (info.avatar_url == nil || info.avatar_url.length == 0) {
        
        entity.imageUrl = nil;
    } else {
        entity.imageUrl = [NSString stringWithFormat:@"%@/%@",[ONEUrlHelper userAvatarPrefix],info.avatar_url];
    }
    return entity;
