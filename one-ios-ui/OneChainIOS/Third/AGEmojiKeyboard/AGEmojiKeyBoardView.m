...
- (instancetype)initWithFrame:(CGRect)frame dataSource:(id<AGEmojiKeyboardViewDataSource>)dataSource {
  ...
  if (self) {
    ...
    [self addSubview:self.emojiPagesScrollView];
      ...
      [_sendButton setTitle:NSLocalizedString(@"button_send", @"Send") forState:UIControlStateNormal];
      [_sendButton setBackgroundColor:[UIColor colorWithHex:THEME_COLOR]];
      [_sendButton addTarget:self action:@selector(sendEmojiAction) forControlEvents:UIControlEventTouchUpInside];
      
      [_sendButton setFrame:CGRectMake(CGRectGetMaxX(self.frame) - 60, CGRectGetMaxY(self.emojiPagesScrollView.frame) + 5, 50, 30)];
...
- (void)sendEmojiAction
{
    if ([self.delegate respondsToSelector:@selector(emojiKeyBoardViewDidPressSendButton:)]) {
        ...
        [self.delegate emojiKeyBoardViewDidPressSendButton:self];
    }
}
